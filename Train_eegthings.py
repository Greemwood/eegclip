...
class BrainDiffusionPrior(nn.Module):
    def __init__(self,prior_model=None,eeg_model=None,eeg_text_model=None,device='cuda'):
        ...
        self.i = 0
    def forward(self, eeg_data, text_features, img_features, alpha, imglabels):
        
        # print(self.i)
        self.i +=1
        h_embeds = img_features
        
        loss_img,eeg_features = self.eeg_model(eeg_data,img_features,  text_features, alpha,imglabels)
...
class EEGViT_pretrained(nn.Module):
    def __init__(self,depth=4,num_attention_heads=8):
        ...
        config.update({'image_size': (1,512)})
        config.update({'patch_size': (1,128)})
        config.update({'num_hidden_layers': depth})
        config.update({'hidden_size': hidden_size[1]})
        config.update({'num_attention_heads': num_attention_heads})
    ...
    def forward(self,x,img_features,text_features,alpha,imglabels):
        x = x.unsqueeze(1)
        x=self.conv1(x)
...
def train_model(args, BrainPrior, dataloader, optimizer,lr_scheduler, device, text_features_all, img_features_all):
    ...
    for batch_idx, (eeg_data, labels, text, text_features, img, img_features) in enumerate(dataloader):
        ...
        loss,loss_img, loss_prior, eeg_features_proj= BrainPrior(eeg_data, text_features, img_features, alpha, labels)
...
def evaluate_model(args, BrainPrior, dataloader, device, text_features_all, img_features_all, k):
    ...
    with torch.no_grad():
        for batch_idx, (eeg_data, labels, text, text_features, img, img_features) in enumerate(dataloader):
            ...
            loss, loss_img, loss_prior, eeg_features= BrainPrior(eeg_data, text_features, img_features, alpha, labels)
...
def main():  
    ...
    # for (vitdepth,vitheads,dpdepth,dpheads) in [(4,8,4,16),(2,8,4,16),(8,8,4,16),(4,4,4,16),(4,16,4,16),
    #                                             (4,8,2,16),(4,8,8,16),(4,8,4,8),(4,8,4,32)]:
    for (vitdepth,vitheads,dpdepth,dpheads) in [(4,8,4,16)]:
        for sub in subjects:                    
            train_dataset = EEGDataset(data_path, subjects=[sub] if config['insubject'] else [], exclude_subject=sub if not config['insubject'] else None, train=True,average = False)
            ...
            eeg_model = EEGViT_pretrained(depth=vitdepth,num_attention_heads=vitheads)
            eeg_text_model = None
            # eeg_text_model = EEGtext_pretrained()
            from diffusers import PriorTransformer, UNet2DConditionModel
            # diffusion_prior = PriorTransformer(num_attention_heads = 16, attention_head_dim = 1024//16, embedding_dim=1024, encoder_hid_proj_type = None, num_embeddings = 1, additional_embeddings = 0, num_layers = 8)
            # diffusion_prior = DiffusionPriorUNet(cond_dim=1024, dropout=0.2)
            num_attention_heads = dpheads
            ...
            BrainPrior = BrainDiffusionPrior(eeg_model=eeg_model,eeg_text_model=eeg_text_model, prior_model=diffusion_prior, device=device)
            ...
            lr_scheduler_eeg = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_eeg, mode='min', factor=0.1, 
            ...
            if args.rank==0:
                ...
                shutil.copy('Generation_eegthings.py', outdir)
            resume_from_ckpt = False
            if os.path.exists(os.path.join(outdir, '200.pth')):
                ckpt_path = os.path.join(outdir, '200.pth')
            ...
            results = main_train_loop(sub, epoch, current_time, model,  train_loader, test_loader,  optimizer, lr_scheduler, device, text_features_train_all, text_features_test_all, img_features_train_all, img_features_test_all, config, args, logger=config['logger'],outdir = outdir)
