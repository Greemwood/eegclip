...
class EEGDataset():
    ...
    """
    def __init__(self, data_path, exclude_subject=None, subjects=None, train=True, time_window=[0, 1.0], classes = None, pictures = None, average = False):
        ...
        if self.average:
            reshaped_tensor = self.data.view(16540, 4, 63, 512)
            self.data = reshaped_tensor.mean(dim=1)
            self.labels = self.labels.view(16540, 4)
        ...
        if self.classes is None and self.pictures is None:
            ...
            features_filename = os.path.join(f'no_norm_ViT-H-14_features_train.pt') if self.train else os.path.join(f'no_norm_ViT-H-14_features_test.pt')
            # features_filename = ''
            ...
            self.text_features = self.Textencoder(self.text)
            self.img_features = self.ImageEncoder(self.img)
        print(f"Data tensor shape: {self.data.shape}, label tensor shape: {self.labels.shape}, text length: {len(self.text)}, image length: {self.img_features.shape}")
    def load_data(self):
        data_list = []
        label_list = []
        texts = []
    ...
    def __getitem__(self, index):
        ...
        # import random
        # if self.train:
        #     text_index = random.randint(0, 1653)
        #     img_index = random.randint(0, 16539)
        # else:
        #     text_index = random.randint(0, 199)
        #     img_index = random.randint(0, 199)
...
if __name__ == "__main__":
    ...
    # train_dataset.mod_data()
    # train_dataset.img_aug()
    test_dataset = EEGDataset(data_path, subjects = ['sub-01'], train=False)
    # test_dataset.img_aug()
