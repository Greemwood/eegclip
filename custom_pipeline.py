...
def generate_ip_adapter_embeds(
    ...
    img_lowlevel: Optional[torch.FloatTensor] = None,#++
    ...
    **kwargs,
    ...
    #     input_pooled_prompt_embeds = pooled_prompt_embeds
    # else:
    ...
    # if input_pooled_prompt_embeds is not None:
    #     pooled_prompt_embeds = input_pooled_prompt_embeds
    ...
    if ip_adapter_image is not None:
        ...
        # image_embeds, negative_image_embeds = encode_image(ip_adapter_image, device, num_images_per_prompt)
        if self.do_classifier_free_guidance:
    ...
    if not output_type == "latent":
        ...
        if img_lowlevel != None:
            image = 0.75*image +0.25*img_lowlevel
...
class Generator4Embeds:
    ...
    def __init__(self, num_inference_steps=1, device='cuda') -> None:
        ...
        # )
        # pipe = DiffusionPipeline.from_pretrained("../stabilityai/sdxl-turbo", torch_dtype=torch.float16, variant="fp16",image_encoder=image_encoder)
        ...
        pipe.encode_prompt = encode_prompt.__get__(pipe)
        # load ip adapter
    ...
    def generate(self, image_embeds,prompt_embeds , text_prompt=None,ae_preds = None,  generator=None,blurred_image = None,text_prompt_embeds=None):
        # image_embeds = image_embeds.to(device=self.device, dtype=self.dtype)
        pipe = self.pipe
        # blurry_recons = pipe.vae.decode(ae_preds / pipe.vae.config.scaling_factor, return_dict=False)
        # blurry_recons = pipe.vae.decode(ae_preds.half()/0.18215).sample / 2 + 0.5
        # generate image with image prompt - ip_adapter_embeds
        image = pipe.generate_ip_adapter_embeds(
            prompt=text_prompt, 
            # latents = ae_preds,
            prompt_embeds = prompt_embeds,
            pooled_prompt_embeds = text_prompt_embeds,
            img_lowlevel = blurred_image,
            # ip_adapter_image = blurred_image,
