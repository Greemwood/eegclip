...
class EEGDataset(Dataset):
    ...
    # Constructor
    def __init__(self, eeg_signals_path, subject = 4):
        # Load EEG signals
        loaded = torch.load(eeg_signals_path)
        ll=[]
        for i in loaded['dataset'] :
            ll.append(i['eeg'].shape[1])
        if subject!=0:
        ...
        # Compute size
        self.size = len(self.data)
        self.img_features = torch.load('nonorm_imagenet_ViT-H-14_features.pt')['img_features']
    ...
    def __getitem__(self, i):
        ...
        img_feature = self.img_features[self.data[i]["image"]]


        # return {'eeg': eeg, 'label': label, 'img_feature': img_feature}
        return  eeg, label, img_feature
        # Return
        # return eeg, label
...
class Splitter:
    ...
    def __init__(self, dataset, split_path, split_num=0, split_name="train", subject=4):
        ...
        self.num_voxels = 440
        self.data_len = 512
        self.img_features = torch.stack([self.dataset.img_features[self.dataset.data[i]["image"]] for i in self.split_idx])
        self.images = [self.dataset.images[self.dataset.data[i]["image"]] for i in self.split_idx]
...
def create_EEG_dataset(eeg_signals_path='../dreamdiffusion/datasets/eeg_5_95_std.pth', 
    ...
    dataset_train = EEGDataset(eeg_signals_path,  subject )
    dataset_val = EEGDataset(eeg_signals_path,  subject)
    dataset_test = EEGDataset(eeg_signals_path,  subject)
