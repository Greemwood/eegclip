def epoching(args, data_part, seed):
	...
	chan_order = ['Fp1', 'Fp2', 'AF7', 'AF3', 'AFz', 'AF4', 'AF8', 'F7', 'F5', 'F3',
				  ...
				  'P7', 'P5', 'P3', 'P1', 'Pz', 'P2', 'P4', 'P6', 'P8', #47 -55

				  'PO7', 'PO3', 'POz', 'PO4', 'PO8','O1', 'Oz', 'O2']#56-63
	...
	for s in range(args.n_ses):
		...
		epochs = mne.Epochs(raw, events, tmin=-.2, tmax=1.0, baseline=(None,0),
			preload=True)
		# epochs = mne.Epochs(raw, events, tmin=0.0, tmax=1.0, baseline=(0,0),
		# 	preload=True)
		...
		# Resampling
		if args.sfreq < 1000:
			epochs.resample(args.sfreq,npad = 0)
		...
		del data
		epoched_data.append(sorted_data[:, :, :, 103:])
...
def save_prepr(args, whitened_test, whitened_train, img_conditions_train,
	...
	save_dir = os.path.join(args.project_dir,
		'Preprocessed_data_512Hz', 'sub-'+format(args.sub,'02'))
